  ...
  class FieldDataTest < ActiveSupport::TestCase
    describe 'when a record is a compound' do
      it 'finds children' do
        ...
        item_klass.expect :find, item_klass_obj, [parent_id]
        item_klass_obj.expect :is_compound?, true, []
        children_klass.expect :new, children_klass_obj, [parent_id: parent_id, :check_exists=>false, :fq=>["translation:[* TO *]"]]
        children_klass_obj.expect :find, [child_klass], []

        data = FieldData.new(parent_id: parent_id,
                             field: 'translation',
