    ...
    describe 'when an item is not a compound' do
      describe 'and it is not an image' do
        it 'does not have a manifest' do
          _(IiifManifest.new(viewer_type: 'foo').url).must_equal(false)
        end
      end
      describe 'and it is an image' do
        it 'has a manifest' do
          _(IiifManifest.new(viewer_type: 'image').url).must_equal("https://cdm16022.contentdm.oclc.org/iiif/info/MISSING_COLLECTION/MISSING_ITEM_ID/manifest.json")
        end
      end
    end
