  ...
  class DownloadTest < ActiveSupport::TestCase
    describe 'when requesting downloads for an image viewer' do
      it 'produces the correct image urls' do
        ...
        _(download.urls).must_equal([{:url=>"http://cdm16022.contentdm.oclc.org/digital/iiif/p16022coll142/147/full/150,150/0/default.jpg", :label=>"(150 x 150 Download)"}])
      end
    end
    ...
    describe 'when requesting downloads for an pdf viewer' do
      it 'produces an cdm file download' do
        ...
        download = Download.new(item: item)
        _(download.urls).must_equal 'http://cdm16022.contentdm.oclc.org/utils/getfile/collection/p16022coll142/id/147/filename'
      end
    end
  end
end
